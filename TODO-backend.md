...
- 每個 Z step 只 `moveZ` 一次，四種共用；Z 只在整個圖層做完後復位一次。**開啟折射補償後仍然成立** —— 補償是在切片前把 mesh 扭曲掉的，ladder 的數字本身就是機器 Z（見 I-6）。
...
`refraction-compensator.h/.cpp`。雷射穿過工件上表面時會折射，造成兩件事，**量級差非常多**：

1. **深度**：會聚光束進入較密介質後收斂變慢，焦點會落得比空氣中更深。要把焦點放在真實深度 `d`，空氣中的名義焦平面只能放在 `t = d / n`（光軸上就是課本的視深關係）。這是一階效應，n=1.52 時等於深度的 34%，**幾十 mm**。
2. **位置**：離軸光線斜射入表面、往法線偏折，橫向漂移比空氣中少。但深度補償是**靠抬頭**做的，抬頭又把橫向幾何用同一個 1/n 放大回去，兩者幾乎抵消 —— 殘差在 70mm 視場、100mm 深時只有 **0.05mm**。

模型（距離都從振鏡光學中心 P 起算，半徑從視場中心量）：

```
R = F * tan(theta1)                          // 命令值（XY 校正在設計焦平面上）
r = (F - t) * tan(theta1) + d * tan(theta2)  // 焦點實際落點
sin(theta1) = n * sin(theta2)
t = d * cos²(theta1) / (n * cos²(theta2))    // 平面界面上會聚錐的 sagittal 焦點，光軸上 = d/n
```

⚠️ **關鍵是 `F - t` 不是 `F`**：Z 軸移動的是頭，補償本身把頭抬高了 `H - t`，光學中心到上表面的距離因此變成 `F - t`。`t` 最大到 `H / n`，和 F 同一個量級。

### 深度：把補償收進「切片行為」，不是切完再修

固定一個機器 Z，雷射打得到的**不是一個平面**，而是一個往邊緣越來越深的淺碗（100mm 深時邊緣比中心深 1.08mm，等效曲率半徑約 0.87m，曲率正比於深度）。所以切平面、再回頭幫每個點算 Z，等於跟機器對著幹 —— **每個打點都要一次 `moveZ`**。

改成**切片前先把 mesh 沿 Z 扭曲**（`stl::MeshWarp`，實作在 `RefractionCompensator::warpZ()`）：

```
w(x, y, z) = H - t,  t = (H - z) * cos²(theta1) / (n * cos²(theta2)),  theta1 = atan(r / F)
```

`w` 就是「要打到這個點，頭該停在哪」。在 w 空間切等距平面 ⇒ **一層剛好對應一個機器 Z**，`outputLayerStlGcode()` 的 ladder 完全不用改，它拿到的數字本來就已經是補償後的機器 Z（`Layer::z_geometry` 在有 warp 時就是機器 Z，`z_compensated` 已移除）。

- `∂w/∂z = cos²θ1/(n cos²θ2) > 0` 恆成立，slicer 的 active-edge 掃描不受影響
- 補償關掉、或 n <= 1、或沒有材料高度時，warp 是 identity，行為與先前完全一樣
- 焦距未知時 `theta1 = 0`，warp 退化成單純的 z 縮放（仍然是正確的一階補償，只是沒有視場角修正）

### ⚠️ 三角形細分（warp 是非線性的）

warp 在 XY 上是非線性的，所以三角形的直邊在 w 空間會變成曲線，弦與真實曲面的偏差約 `k·L²/8`（`k = (d/n)(1 - 1/n²)/F²`）。`Slicer::prepare()` 在套 warp **之前**，會把過長的邊用**三邊取中點切成四個三角形**的方式遞迴細分（`MeshWarp::maxEdgeLength()` 給門檻，遞迴上限 6 層 = 一個三角形最多 4096 個）。

門檻由容差反推：`L = sqrt(8 * 0.01mm / k)`，F=254、H=100、n=1.52 時是 11.8mm。對照組：一條沒細分的 70mm 長邊（方塊模型的側面很容易出現）偏差 0.354mm，細分後 < 0.01mm。細分只在 warp 開啟時發生，而且只切**超過門檻**的三角形 —— 一般模型的三角形都遠小於 11.8mm，數量不會變。

### 【給前端】層高語意

⚠️ **層高仍然是「模型上的真實深度間距」，前端不用改任何東西**，這裡只是說明後端做了什麼換算，以免對照 log 時被層數/Z 值嚇到：

- 切片是在**機器 Z 空間**做的，而機器 Z 的一步約等於真實深度的 `1/n`
- 所以後端把層高乘上 `1/n` 之後才拿去切（`layerHeightScale()`）
- **層數不變**：模型高 100mm、層高 0.1mm 仍然是 1000 層
- 但 **log / gcode 裡的 Z 值不是模型高度**：n=1.52、H=100mm 的工件，模型底部那一層的機器 Z 是 34.2mm 而不是 0
- 視場邊緣的真實層厚會比中心厚約 1%（100mm 深時），遠低於任何有意義的層高

### 橫向：留在輸出端

橫向補償是**圖案的畸變**、不是幾何的畸變，所以留在 emitter：每層用一次正掃建 `r -> scale` 表（256 點），每個點二分搜尋 + 內插。

- 打線 / 掃描填充：長線段切成 2mm 一段，否則徑向畸變會讓直線微彎
- 打點：每個點各自對應，**中間絕對不能插點**（會變成多餘的點、間距也錯）
- 沒有焦距時整個橫向項自動關閉（scale 恆為 1）

### ⚠️ 參考實作（GPT 的 `calculateOpticalPosition`）的三個問題

已移除，改成上面的模型。原因：

1. **`solveTheta1()` 不存在** —— 那份 code 根本編不過
2. **`r = 0` 的特例 `(n-1)*d` 與自己的通式不一致**：通式在 `r -> 0` 的極限是 `d/n`（100mm 深、n=1.52 → 65.8 vs 特例的 52）
3. **最嚴重：把光學中心到上表面的距離寫成固定的 `F`**，等於假設補償 Z 的時候頭沒有動。同一組數字下，它會把 44.9mm 的目標半徑壓縮到 35.7mm —— **9mm 的偏移**。換成 `F - t` 之後，橫向補償才回到 0.05mm 的量級

### 仍待確認

- **F 從哪裡來**：後端 machine settings 沒有焦距欄位，現在靠 convert param
- **對焦原點**：程式假設使用者對焦在工作平台（PM 08/06 定案），工件底部貼平台、上表面在 H
- 模型是一階近似：真實振鏡沒有單一 pivot、f-theta 鏡是 `R = f * theta` 而不是 `F * tan(theta)`、斜射的會聚錐有兩個焦點（像散），而且真實鏡頭本身就有場曲。這些誤差都比模型內部的取捨大，**方向與深度尺度務必先用小工件實機驗證**
...
| param | 說明 |
| --- | --- |
| `refraction_compensation` | bool，折射率補償開關，**省略時預設開** |
| `refractive_index` | 工件折射率 n，<= 1 等於不補償（預設 1.0） |
| `material_height` | 材料高度 H（mm），工件上表面距離對焦原點（工作平台）的高度。**沒有它就完全不能補償** |
| `focal_length` | 焦距 F（mm），光學中心到設計焦平面。**省略則退化成「只補深度、不補 XY」** |

⚠️ `material_height` 與 `focal_length` 是**新的需求**：TODO.md A-3 原本寫「材料形狀/尺寸不傳給後端」，但折射補償一定要知道上表面在哪。F 則是機器的光學規格（鏡頭焦距），後端的 machine settings 裡沒有這個欄位，目前只能由前端隨 convert params 送。
