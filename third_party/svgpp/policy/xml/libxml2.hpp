    ...
    namespace xml {
      ...
      struct attribute_iterator<xmlAttr *> {
        ...
        static BOOST_SCOPED_ENUM(detail::namespace_id) get_namespace(iterator_type xml_attribute) {
          ...
          boost::iterator_range<char const *> ns_uri = boost::as_literal(
               reinterpret_cast<const char *>(xml_attribute->ns->href));
      ...
      struct element_iterator<xmlNode *> {
        ...
        static string_type get_string_range(element_name_type const &str) {
          return str;
        }

        static string_type get_string_range(element_text_type const &str) {
        ...
        static element_name_type get_local_name(iterator_type xml_element) {
          ...
          //TODO (xml char here is not freed)
          return boost::as_literal(reinterpret_cast<const char *>(local_name));
        }
            ...
            switch (xml_element->type) {
              case XML_ELEMENT_NODE: {
                if (xml_element->ns == NULL)
                  break;
                boost::iterator_range<char const *> ns_uri = boost::as_literal(
                     reinterpret_cast<const char *>(xml_element->ns->href));
