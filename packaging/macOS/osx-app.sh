...
CODE_SIGN_IDENTITY="FLUX TECHNOLOGY CORP. (4Y92JWKV94)"
...
sparkle_version="2"
sparkle_frameworks_dir="/Library/Frameworks/Sparkle.framework"
...
#
"" "$bundle" -qmldir=/Users/estherfu/Desktop/swiftray/src/windows/qml -no-strip -always-overwrite -verbose=2 || exit 1

#
# The build process added to the Swiftray binary an rpath
...
if [ -n "$CODE_SIGN_IDENTITY" ] ; then
  ...
  if [ "$sparkle_version" == "2" ] ; then
	...
	echo "Signing LCS libraries"
	codesign_file "$pkgexec/libutils.dylib"
	codesign_file "$pkgexec/libtransfer.dylib"
	codesign_file "$pkgexec/libparam.dylib"
	codesign_file "$pkgexec/liblcs2dll.dylib"
	codesign_file "$pkgexec/libexecutor.dylib"
	codesign_file "$pkgexec/libcrypto6.0.dylib"
	codesign_file "$pkgexec/libcrypto3.dylib"
	codesign_file "$pkgexec/libBslCal.dylib"
