...
if not exist "%ORT_DIR%\include\onnxruntime_cxx_api.h" (
  ...
  curl.exe -L -o "%TEMP%\%ORT_PKG%.zip" "https://github.com/microsoft/onnxruntime/releases/download/v%ORT_VER%/%ORT_PKG%.zip" || exit /b 1
...
for %%m in (mobile_sam.encoder.onnx mobile_sam.decoder.onnx) do (
  if not exist "resources\models\%%m" (
    if exist "..\mini-sam\models\%%m" (
      ...
      echo Downloading %%m
      curl.exe -L -o "resources\models\%%m" "%MODELS_URL%/%%m" || exit /b 1
    )
  )
)
