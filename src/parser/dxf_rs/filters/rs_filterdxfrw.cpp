...
#include "parser/dxf_rs/engine/rs_block.h"
...
#include "parser/dxf_rs/engine/rs_system.h"
...
bool RS_FilterDXFRW::fileImport(RS_Graphic& g, const QString& file, RS2::FormatType type) {
    ...
    // for(unsigned int i = 0; i < graphic->count(); ++i) {
    //     std::cout << std::endl << "the rtti = " << graphic->entityAt(i)->rtti() << std::endl;
    //     std::cout << "in layer name = " << graphic->entityAt(i)->getLayer()->getName().toStdString() << std::endl;
    //     if(graphic->entityAt(i)->rtti() == RS2::EntityPolyline || 
    //         graphic->entityAt(i)->rtti() == RS2::EntityInsert) {
    //         RS_EntityContainer *tmp_container = (RS_EntityContainer*)graphic->entityAt(i);
    //         for(unsigned int j = 0; j < tmp_container->count(); ++j) {
    //             std::cout << "  the rtti = " << tmp_container->entityAt(j)->rtti() << std::endl;
    //         }
    //     }

    //     std::cout << "the entity = " << *graphic->entityAt(i) << std::endl;
    // }
