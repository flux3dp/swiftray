...
class ToolpathExporterFcode : public QObject {
 ...
 public:
  enum class HardwareType { beamo, Beambox, BeamboxPro, HEXA, Ador, BB2, RF40, RF70 };
...
public Q_SLOTS:
 ...
 private:
  void parseParam(const QJsonObject* paramPtr) {
    ...
    } else if (model == "fhx2rf3" || model == "fhx2rf4") {
      hardware_ = HardwareType::RF40;
      is_v2_ = true;
    } else if (model == "fhx2rf6" || model == "fhx2rf7") {
      hardware_ = HardwareType::RF70;
      is_v2_ = true;
    } else {
      // default beambox
